#!/usr/bin/env python3
"""Render a fixed-format automation acceptance report from full_loop dry-run output.

Usage:
  python scripts/render_automation_acceptance_report.py \
      --repo-root . \
      --input .hgs/full_loop_dry_run.json \
      --output .hgs/automation_execution_acceptance_report.json
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Render automation execution acceptance report from dry-run output")
    parser.add_argument("--repo-root", default=".", help="Repository root")
    parser.add_argument("--input", default=".hgs/full_loop_dry_run.json", help="Dry-run JSON path")
    parser.add_argument("--output", default=".hgs/automation_execution_acceptance_report.json", help="Rendered report path")
    parser.add_argument("--strict", action="store_true", help="Exit non-zero when automation execution status is not pass")
    ...
    report = {
        ...
        "judgements": {
            ...
            "validation_experience_rereview_clearance_path_established": "yes" if all([
                dry_run.get("validation_plan_created"),
                dry_run.get("experience_plan_created"),
                dry_run.get("rereview_path_created"),
                dry_run.get("clearance_loop_created"),
            ]) else "no",
            "continue_until_open_issue_zero_default_rule": "yes" if clearance_loop.get("rule") == "continue until open_issue_count = 0" else "no",
            "still_asking_user_to_pick_a_or_b": "no",
            "still_stuck_in_suggestion_mode": "no",
        }
    }
