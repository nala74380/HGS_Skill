#!/usr/bin/env python3
"""Verify Release assembly acceptance from assembly ledger.

Usage:
  python scripts/verify_release.py \
      --repo-root . \
      --ledger .hgs/assembly_ledger.json \
      --output .hgs/assembly_report.json
"""
...
REQUIRED_SCORING_DOCS = [
    "Release/docs/HGS_全局检查与清理评分报告.md",
]
REQUIRED_LEDGER_DOCS = [
    "Release/docs/HGS_全局扣分点问题清单与派单台账.md",
]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify HGS Release acceptance from assembly ledger")
    parser.add_argument("--repo-root", default=".", help="Repository root")
    parser.add_argument("--ledger", default=".hgs/assembly_ledger.json", help="Assembly ledger JSON path")
    parser.add_argument("--output", default=".hgs/assembly_report.json", help="Acceptance report JSON path")
    parser.add_argument("--strict", action="store_true", help="Exit non-zero when acceptance fails")
    args = parser.parse_args()

    repo_root = Path(args.repo_root).resolve()
    ledger_path = repo_root / args.ledger
    if not ledger_path.exists():
        raise SystemExit(f"Assembly ledger not found: {ledger_path}")
    ...
    entrypoint_ok = f"Release/{ledger.get('entrypoint')}" in loaded or ledger.get("entrypoint", "").startswith("Release/") and ledger.get("entrypoint") in loaded
