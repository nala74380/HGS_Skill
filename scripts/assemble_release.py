...
def main() -> int:
    ...
    registered = [normalize_path(args.manifest), normalize_path(manifest['entrypoint'])]
    ...
    release_files = sorted(str(p.relative_to(root).as_posix()) for p in (root / 'Release').rglob('*') if p.is_file())
    allowed = set(deduped)
    ...
    ledger = {
      ...
      'action_protocol': policy.get('action_protocol'),
      'clearance_protocol': policy.get('clearance_protocol'),
      ...
      'active_actions_count': len(policy.get('active_actions', [])),
      'active_action_batches': list(policy.get('active_action_batches', [])),
